...
def sample_iid_train_data(train_dataset, no_participants):
    all_indices = list(range(len(train_dataset)))
    random.shuffle(all_indices)
    per_participant_list = defaultdict(list)
    samples_per_participant, remainder = divmod(len(all_indices), no_participants)
    start = 0
    for i in range(no_participants):
        end = start + samples_per_participant
        if i < remainder:
            end += 1
        per_participant_list[i] = all_indices[start:end]
        start = end
    return per_participant_list
