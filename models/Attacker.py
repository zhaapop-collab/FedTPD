...
def attacker(list_mal_client, num_mal, attack_type, dataset_train, dataset_test, dict_users, net_glob, args, a3fl, myattack, epoch, idx=None):
    num_mal_temp=0
    if args.ada_mode == 20:
    ...
    if attack_type == "dba":
        ...
        args.dba_sign += 1
    local = LocalMaliciousUpdate(args=args, dataset=dataset_train, idxs=dict_users[idx], order=idx, dataset_test=dataset_test, a3fl=a3fl, myattack=myattack)
    if attack_type == "layerattack_ER_his" or attack_type == "LFA" or attack_type == "LPA":
        ...
        w, loss = local.train(
            net=copy.deepcopy(net_glob).to(args.device), test_img=test_img, epoch=epoch)
