...
class LocalUpdate(object):
    ...
    def train(self, net, idx=None, defense=None, epoch=None):
        ...
        if defense is not None:
            punish_loss = defense.train(self.ldr_train, net, optimizer, idx, epoch)
            if punish_loss != -1:
                punish_losses.append(punish_loss)
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                # if defense is not None:
                #     loss += defense.punish_loss(idx, net, images, labels, epoch)

                loss.backward()
                optimizer.step()
                batch_loss.append(loss.item())
            # if defense is not None:
            #     defense.modify_punish_factor(punish_loss, sum(batch_loss)/len(batch_loss), idx, epoch)

            epoch_loss.append(sum(batch_loss)/len(batch_loss))
        if defense is not None and idx not in defense.malicious_clients and epoch >= defense.start_epoch:
            print("Round: " + str(epoch) + " client: " + str(idx) + "  punish_loss: " + str(sum(punish_losses)/len(punish_losses)))
