...
def test_a3fl(model, data_source, args, a3fl):
    model.eval()
    with torch.no_grad():
        correct = 0
        num_data = 0.
        for batch_id, (data, targets) in enumerate(data_source):
            data, labels = data.to(args.device), targets.to(args.device)
            # 对输入数据进行中毒处理
            data, targets = a3fl.poison_input(data, targets, eval=True)
            # 前向传播
            output = model(data)
            # 获取预测结果
            ...
            for i in range(output.size(0)):
                # 检查当前样本是否符合攻击条件
                if test_or_not(args, labels[i].item()):
                    # 检查预测结果是否正确
                    if pred[i].item() == args.attack_label:
                        correct += 1
                    num_data += 1
...
def test_img(net_g, datatest, args, test_backdoor=False):
    ...
    for idx, (data, target) in enumerate(data_loader):
        ...
        if test_backdoor and args.attack != "a3fl":
