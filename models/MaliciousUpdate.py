...
class LocalMaliciousUpdate(object):
    def __init__(self, args, dataset=None, idxs=None, attack=None, order=None, malicious_list=None, dataset_test=None, a3fl=None, myattack=None):
        self.a3fl = a3fl
        self.myattack = myattack
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.selected_clients = []
        self.ldr_train = DataLoader(DatasetSplit(
            dataset, idxs), batch_size=self.args.local_bs, shuffle=True, drop_last=True)
    ...
    def train(self, net, epoch=None, test_img = None):
        if self.attack == 'badnet':
            return self.train_malicious_badnet(net, epoch)
        ...
        elif self.attack == 'a3fl':
            return self.train_a3fl_attack(net, epoch)
        elif self.attack == 'myattack':
            return self.train_myattack_attack(net, epoch)
        elif self.attack == 'SF':
            return self.train_sf_attack(net)
        elif self.attack == 'LF':
            return self.train_lf_attack(net)
        elif self.attack == 'FR':
            return self.train_fr_attack(net)
        elif self.attack == 'LF+SF':
            return self.train_lf_sf_attack(net)
        else:
            print("Error Attack Method")
            os._exit(0)


    def train_lf_sf_attack(self, net):
        # 1. 保存初始模型（全局模型）状态
        global_model = copy.deepcopy(net.state_dict())

        # 2. 执行标签翻转攻击（L-flip）
        lf_model_params, _ = self.train_lf_attack(copy.deepcopy(net))

        # 3. 执行对抗性梯度上升攻击（G-asc）
        sf_model_params, _ = self.train_sf_attack(copy.deepcopy(net))

        # 4. 计算两个扰动：delta_lf, delta_sf
        delta_lf = {}
        delta_sf = {}
        for k in global_model.keys():
            delta_lf[k] = lf_model_params[k] - global_model[k]
            delta_sf[k] = sf_model_params[k] - global_model[k]

        # 5. 将两个扰动相加，得到最终的攻击更新
        attack_model = {}
        for k in global_model.keys():
            attack_model[k] = global_model[k] + delta_lf[k] + delta_sf[k]

        return attack_model, 0

    def train_fr_attack(self, net):
        new_state = {}
        for name, param in net.state_dict().items():
            if torch.is_floating_point(param):
                fake_update = torch.empty_like(param).uniform_(-1, 1)
                new_state[name] = param + fake_update
            else:
                new_state[name] = param  # 对于非浮点参数，保持不变
        return new_state, 0

    def train_lf_attack(self, net):
        net.train()
        # train and update
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                # 进行标签翻转
                flipped_labels = num_classes - labels - 1
                flipped_labels = torch.clamp(flipped_labels, 0, num_classes - 1)

                net.zero_grad()
                log_probs = net(images)
                loss = self.loss_func(log_probs, flipped_labels)
                loss.backward()
                optimizer.step()
    ...
    #                 low=0, high=num_classes, size=labels.size(), dtype=labels.dtype, device=labels.device
    #             )
    #
    #             # 可选：确保随机标签不等于原标签（强扰动）
    #             mask = (random_labels == labels)
    #             while mask.any():
    #                 random_labels[mask] = torch.randint(
    #                     low=0, high=num_classes, size=(mask.sum(),), device=labels.device)
    #                 mask = (random_labels == labels)
    #
    #             net.zero_grad()
    #             log_probs = net(images)
    #             loss = self.loss_func(log_probs, random_labels)
    #             loss.backward()
    #             optimizer.step()
    #             batch_loss.append(loss.item())
    #         epoch_loss.append(sum(batch_loss) / len(batch_loss))
    ...
    def train_sf_attack(self, net):
        ...
        result = {}
        for k in global_model.keys():
            result[k] = 2 * global_model[k] - net.state_dict()[k]

        return result, sum(epoch_loss) / len(epoch_loss)

    def train_myattack_attack(self, net, epoch):
        net.train()
        # train and update
        optimizer = torch.optim.SGD(
            net.parameters(), lr=self.args.lr, momentum=self.args.momentum)
        # epoch_loss = []
        #
        # self.myattack.train(net, self.ldr_train, optimizer)
        #
        # # for iter in range(self.args.local_ep):
        # #     batch_loss = []
        # #     for batch_idx, (images, labels) in enumerate(self.ldr_train):
        ...
        #
        # return net.state_dict(), sum(epoch_loss) / len(epoch_loss)
        loss = self.myattack.train(net, self.ldr_train, optimizer)

        return net.state_dict(), loss

    def train_a3fl_attack(self, net, epoch):
        net.train()
        # train and update
        optimizer = torch.optim.SGD(
            net.parameters(), lr=self.args.lr, momentum=self.args.momentum)
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                images, labels = images.to(
                    self.args.device), labels.to(self.args.device)
                if epoch > self.a3fl.start:
                    images, labels = self.a3fl.poison_input(images, labels)
                net.zero_grad()
                log_probs = net(images)
                loss = self.loss_func(log_probs, labels)
                loss.backward()
    ...
    def train_malicious_badnet(self, net, epoch=None, test_img=None, dataset_test=None, args=None):
        net.train()
        # train and update
        optimizer = torch.optim.SGD(
            net.parameters(), lr=self.args.lr, momentum=self.args.momentum)
